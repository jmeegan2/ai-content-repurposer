...
beforeEach(() => {
  ...
  vi.mocked(processClip).mockResolvedValue({
    clipPath: "/tmp/repurposer-abc/clip-1.mp4",
    thumbnailPath: "/tmp/repurposer-abc/clip-1.jpg",
  });
});
...
describe("runPipeline", () => {
  it("transitions through downloading → transcribing → done on success", async () => {
    ...
    const statuses = updateJob.mock.calls
      .map((c) => c[1].status)
      .filter(Boolean);
  ...
  it("uploads to the correct S3 key", async () => {
    ...
    expect(uploadFile).toHaveBeenCalledWith(
      "raw/job-1/video.mp4",
      "/tmp/repurposer-abc/video.mp4",
    );
  });

  it("sets status to failed and records error message when a step throws", async () => {
    vi.mocked(downloadYouTubeVideo).mockRejectedValue(
  ...
  it("always cleans up the temp directory on success", async () => {
    ...
    expect(rm).toHaveBeenCalledWith("/tmp/repurposer-abc", {
      recursive: true,
      force: true,
    });
  });

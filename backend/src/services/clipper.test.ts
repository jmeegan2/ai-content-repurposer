...
vi.mock('fluent-ffmpeg', () => {
  const createCmd = () => {
    ...
    let errorCb: ((err: Error, stdout: string, stderr: string) => void) | undefined;

    const cmd: any = {
      seekInput: (v: number) => { ffmpegControl.seekInputCalls.push(v); return cmd; },
      duration: (v: number) => { ffmpegControl.durationCalls.push(v); return cmd; },
      videoFilters: (v: string) => { ffmpegControl.videoFilterCalls.push(v); return cmd; },
      videoCodec: () => cmd,
      addOption: () => cmd,
      audioCodec: () => cmd,
      audioBitrate: () => cmd,

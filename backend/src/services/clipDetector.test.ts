...
describe('detectClips', () => {
  ...
  it('returns detected clips from a successful tool response', async () => {
    ...
    expect(clips[0]).toMatchObject({ title: 'Great Moment', startTime: 0.0, endTime: 45.0 });
    expect(clips[1]).toMatchObject({ title: 'Pay Attention', startTime: 50.0, endTime: 90.0 });
  });
  ...
  it('passes the correct model and tool_choice to the API', async () => {
    ...
    expect(call.tool_choice).toEqual({ type: 'function', function: { name: 'report_clips' } });
  ...
  it('throws when the response contains no tool call', async () => {
    ...
    await expect(detectClips(transcript)).rejects.toThrow('Model did not return clip detections');
  });

  it('propagates API errors', async () => {
    mockCreate.mockRejectedValue(new Error('Rate limit exceeded'));

    await expect(detectClips(transcript)).rejects.toThrow('Rate limit exceeded');
  });
});
